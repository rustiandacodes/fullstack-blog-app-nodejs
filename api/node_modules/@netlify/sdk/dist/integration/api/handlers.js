...
export const defaultOnEnableHandler = async (event, context) => {
    ...
    if (!requestBody.siteId && !requestBody.teamId) {
        return {
            statusCode: 400,
    ...
    try {
        if (requestBody.siteId) {
            const { token } = await context.client.enableSiteIntegration(requestBody.siteId, {});
            context.client.netlifyToken = token;
        }
        if (requestBody.teamId &&
            (!requestBody.siteId || requestBody.forceEnableTeam)) {
            const { token } = await context.client.enableTeamIntegration(requestBody.teamId, {});
