import zodLib, { ZodError } from "zod";
import { fromZodError } from "zod-validation-error";
export class ConnectorOptionsSchema {
    schemaDefiner;
    constructor(schemaDefiner) {
        this.schemaDefiner = schemaDefiner;
    }
    buildSchema() {
        try {
            const schema = this.schemaDefiner({ zod: zodLib });
            ...
            if (!(schema instanceof zodLib.ZodSchema)) {
...
connector.defineOptions(({ zod }) => {
  return zod.object({ // <-- zod.object must be returned.
    foo: zod.string().meta({ label: 'Example field' })
            ...
            return this.builtInOptionsSchema().merge(objectSchema);
        }
        catch (e) {
            console.error(e);
            process.exit(1);
    ...
    validate(options, packageName) {
        try {
            this.buildSchema().parse(options);
        }
        catch (e) {
            if (e instanceof ZodError) {
                console.info(`Provided options for connector "${packageName}":`, JSON.stringify(options, null, 2));
                console.error(fromZodError(e, {
                    prefix: `Invalid options for connector "${packageName}"`,
                    prefixSeparator: `:\n  `,
                    issueSeparator: `\n  `,
                }).message);
            }
